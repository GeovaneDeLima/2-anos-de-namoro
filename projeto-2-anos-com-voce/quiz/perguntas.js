export const perguntas = [
    {
        enunciado: 'Qual a data do nosso primeiro beijo?',
        alternativas: ['25/12/2023', '12/06/2023', '08/06/2023', '05/05/2023', '30/04/2023'],
        correta: 2
    },

    {
        enunciado: "Qual o primeiro presente que eu te dei?",
        alternativas: [
            'Uma blusa/cropd preto com as mangas bufentas que teu pai não gostou',
            'Uma caixa de chocolate que quando tu tira atampa ela desmonta mostrando um monte de chocolate.',
            'Um buquê de flores feitos a mão por mim',
            'Um colar dourado que eu comprei quando fui pra furna',
            'Um vestido preto longo que você guardou pra usar na minha formatura'
        ],
        correta: 3
    },

    {
        enunciado: 'Quando começamos a conversar, eu te dei um apelido "carinhoso" que eu usava para te pirraçar. Qual era esse apelido?',
        alternativas: ['Lagartixa', 'Toco de gente', 'Pudin', 'Anderson', 'hozeinhas'],
        correta: 0
    },

    {
        enunciado: 'Qual a música que era praticamente nossa, no início do nosso relacionamento?',
        alternativas: [
            'Sunshine - Delacruz',
            'Pras damas - Oriente', 
            'Mirrors - Justin Timberlake',
            'A Nossa Praia - Matheus & Kauan',
            'Um Dia, Um Adeus - Vanessa da Mata'
        ],
        correta: 1
    },

    {
        enunciado: 'Quando fui na tua casa pela primeira vez, qua a primeira coisa que eu te disse?',
        alternativas: [
            'Comprimentei meus sogros como uma pessoa normal',
            "Te abracei e disse, 'eu te amo' no teu ouvido",
            "me ajoelhei e disse 'Namora comigo?'",
            'Gritei com as meninas para ficarem quietas',
            "Perguntei como um idiota 'esse portão.. é pra pular?'"
        ],
        correta: 4 
    },

    {
        enunciado: "Quando iamos comer algo na lanchonete, era de lei eu pedir uma comida específica. Qual era essa comida?",
        alternativas: [
            'Enroladinho de salsicha ',
            'Coxa de frango com aipin',
            'Cochinha',
            'Pastel de quijo e presunto',
            'Pizza'
        ],
        correta: 1
    },

    {
        enunciado: 'Qual comida eu sempre digo que se parece contigo quando tu ta deitada toda embrulhada em uma ligação comigo?',
        alternativas: [
            'Pizza',
            'Cuzcuz',
            'Frango',
            'Pudim',
            'Muci de maracujá'
        ],
        correta: 3
    },

    {
        enunciado: 'Desde cedo, descidimos quais seriam os nomes dos nossos filhos. Qual seria o nome das nossas filhas caso nascer 2 meninas',
        alternativas: [
            'Rafaela, Allana',
            'Allana, Gabriela',
            'Allana, Geovana junia',
            'Allana, Jucelia',
            'Allana, Yorhana'
        ],
        correta: 4
    },

    {
        enunciado: 'Quando você me pergunta qual presente eu quero ganhar, eu sempre respondo uma coisa. O que eu respondo?',
        alternativas: [
            'Uma camisa do Corinthians preta e branca',
            'Um munitor novo para o meu setup',
            'Um abraço e um beijo é mais que o suficiente',
            'Uma Bross 160 2025',
            'Um pote de whey'
        ],
        correta: 3
    },

    {
        enunciado: 'Você me xinga de uma coisa quando eu te pirraço, e por alguma razão eu amo escutar. Qual é esse xingamento e pq eu gosto tanto',
        alternativas: [
            'Bestão. Eu gosto porque nós dois somos bestas.',
            'Nojento. Eu gosto porque acho a palavra bonita.',
            'Viado. Eu gosto porque sou meio afeminado.',
            'Nojento. Eu gosto porque acho fofo a cara e o jeito que tu fala.',
            'Insuportável. Eu gosto porque amo ser insuportável contigo.'
        ],
        correta: 3 
    },

    

]

